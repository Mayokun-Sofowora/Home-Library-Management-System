...
public class BookController {
    ...
    @PostMapping()
    public ResponseEntity<BibliographicArtifact> createBook(@RequestBody Book book) {
        Book saveBook = bookService.createBook(book);
        return ResponseEntity.status(HttpStatus.CREATED).body(saveBook);
    }

    // Add a new book with authors
    @PostMapping("/add")
    public ResponseEntity<Book> addBook(@RequestBody Book book) {
        Book saveBook = bookService.saveOrUpdateBook(book);
        return ResponseEntity.status(HttpStatus.CREATED).body(saveBook);
    }
